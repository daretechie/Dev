...
class Emailer():
    ...
    has_html = False
    test_send = False
    from_email = 'Hungry Py <email username>'
    template_html = None
    template_name = None

    def __init__(self, subject="", template_name=None, context={}, template_html=None,
                 to_emails=None, test_send=False):
        if template_name is None and template_name is None:
        ...
        if template_html is not None:
            self.has_html = True
            self.template_html = template_name
